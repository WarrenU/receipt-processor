...
func TestGetPoints(t *testing.T) {
	...
	t.Run("Valid receipt flow", func(t *testing.T) {
	...
	t.Run("Invalid receipt returns validation errors", func(t *testing.T) {
		invalidReceipt := []byte(`{
			"retailer": "",
		...
		postResp, err := http.Post(server.URL+"/receipts/process",
			"application/json", bytes.NewBuffer(invalidReceipt))
		assert.NoError(t, err)
		defer postResp.Body.Close()
		...
		err = json.NewDecoder(postResp.Body).Decode(&errs)
		assert.NoError(t, err)
		assert.Greater(t, len(errs), 0)

		for _, e := range errs {
			assert.Contains(t, e, "field")
